...
def get_file_info():
    ...
    for f in files:
        ...
        mtime = os.path.getatime(f)
        array.append({'name': filename, 'mtime': mtime})
...
def get_next_avaliable_port(initial_port):
    ...
    """
    for i in range(initial_port, (2**16)-1):
...
class FileSynchronizer(threading.Thread):
    ...
    def sync(self):
        ...
        try:
            ...
            local_file = get_file_info()
        except (ValueError, e):
            print('Your json data is not valid')
       ...
       # sync and send files json data
        ...
        for file in data_dic:
            ...
            for f in local_file:
                ...
                local_filemtime = f['mtime']

                if (filename != local_filename or (
                    filename == local_filename and filemtime != local_filemtime)):

                    request_file = json.dumps(self.files)
                # send all data
                    self.syncfile(request_file)
...
def syncfile(self, ip, port, filename):
